...
async function searchPhotosDirect(query, accessKey, perPage, orientation) {
...
async function searchPhotos(query, options = {}) {
  ...
  console.log(`[unsplash] search: "${query}" (orientation=${orientation})`);
  ...
  try {
    ...
    if (error) {
      throw new Error(`Unsplash: ${JSON.stringify(error)}`);
    }
    ...
    return searchPhotosDirect(query, accessKey, perPage, orientation);
  }
}
...
async function fetchBackgroundPhoto(query, destPath, options = {}) {
  ...
  const photos = await searchPhotos(query, { perPage, orientation });
  if (!photos.length) {
